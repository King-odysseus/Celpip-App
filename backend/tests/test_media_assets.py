...
def test_listening_seed_is_idempotent_complete_and_integrity_checked():
    ...
    assert "created 168" in first.getvalue()
    assert "created 0" in second.getvalue()
    assert TaskType.objects.filter(skill="listening").count() == 6
    assert ContentItem.objects.filter(task_type__skill="listening").count() == 168
    assert ContentVersion.objects.filter(item__task_type__skill="listening").count() == 168
    ...
    assert listening_questions.count() == 672
    assert listening_choices.count() == 2688
    assert MediaAsset.objects.count() == 168
